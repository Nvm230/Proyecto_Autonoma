...
class BrainNodeNav2(Node):
    def __init__(self):
        super().__init__('brain_node_nav2')
    ...
    def wait_for_nav2(self):
        ...
        self.get_logger().info('✅ SETUP NAV2 LISTO - SISTEMA OPERATIVO ✅')
        self.get_logger().info('=========================================')
    ...
    def voice_cmd_callback(self, msg):
        ...
        if cmd.startswith("buscar"):
            ...
            self.get_logger().info(f'--- BÚSQUEDA NAV2 (MAPA INTELIGENTE): {objeto} ---')
            self.iniciar_exploracion_nav2()
            ...
            self.detener_exploracion()
            self.get_logger().info('Motores frenados y exploración cancelada.')
    ...
    def object_detected_callback(self, msg):
        ...
        if match_found:
            self.get_logger().info(f'¡OBJETO OBJETIVO ENCONTRADO POR YOLO ({objeto_detectado})! Deteniendo Nav2...')
            self.target_object = None
            self.detener_exploracion()

    def iniciar_exploracion_nav2(self):
        self.detener_exploracion()
        self.get_logger().info('Borrando el mapa de SLAM (Amnesia) para re-explorar...')
        # Resetear el mapa de SLAM
        subprocess.Popen(['ros2', 'service', 'call', '/slam_toolbox/reset', 'std_srvs/srv/Empty'])
        
        self.get_logger().info('Esperando 6 segundos a que Nav2 regenere los costmaps...')
        time.sleep(6.0)
        
        self.get_logger().info('Lanzando Explore Lite...')
        use_sim_time = str(self.get_parameter('use_sim_time').value).lower()
        self.explore_process = subprocess.Popen([
            'ros2', 'launch', 'explore_lite', 'explore.launch.py', f'use_sim_time:={use_sim_time}'
        ], preexec_fn=os.setsid)

    def detener_exploracion(self):
        # Freno de emergencia físico
        stop_msg = Twist()
